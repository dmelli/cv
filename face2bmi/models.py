...
class FacePrediction(object):
    ...
    def __init__(self, img_dir, model_type = 'vgg16'):
        ...
        self.detector = MTCNN()
        if model_type in ['vgg16','vgg16_fc']:
            self.version = 1
        else:
            self.version = 2
    
    def define_model(self, hidden_dim = 128, drop_rate=0.0):
    ...
    def evaulate(self, valid_data):
        ...
        metrics = {'bmi_rmse':rmse(bmi[:,0], valid_data.bmi.values), 
                   'age_rmse':rmse(age[:,0], valid_data.age.values), 
    ...
    def predict_df(self, img_dir, sex_thresh = 0.5):
        ...
        res['sex'] = res['sex'].map(lambda i: 'Male' if i > sex_thresh else 'Female')
        
        return res
    
    def predict_faces(self, img_path, show_img = True, color = "white", fontsize = 12, 
                      confidence = 0.95, fig_size = (16,12), sex_thresh = 0.5):
        ...
        if show_img:
            ...
            for idx, box in enumerate(boxes):
                ...
                ax.add_patch(rect)
                ax.text(box_x, box_y, 
                        'BMI:{:3.1f}\nAGE:{:02.0f}\nSEX:{:s}'.format(bmi[0,0], age[0,0], 'M' if sex[0,0] > sex_thresh else 'F'),
